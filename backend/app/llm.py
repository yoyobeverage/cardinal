...
- target_apy: if the user names a specific yield/APY/return they want for the portfolio ("give me 7% yield", "I want 7%", "target 8% APY", "aim for 5% returns"), set target_apy to that bare number ("7%" -> 7.0). This is the blended portfolio yield they want hit exactly, NOT a per-protocol floor. Leave null if they don't name a number. Still extract their other preferences (anchors, lens_weights, concerns) normally - the target combines with all of them.
